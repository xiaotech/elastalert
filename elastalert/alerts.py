...
#            self.body_labelname: self.create_alert_body(matches)})
        ...
        label = self.get_custom_string(self.labels,matches[0])
        annotation = self.get_custom_string(self.annotations,matches[0])
        ...
        payload = json.dumps([{
            ...
            'labels': label,
            'startsAt':"{}".format(start_time.strftime("%Y-%m-%dT%H:%M:%S.%fZ")),
            'endsAt':"{}".format(end_time.strftime("%Y-%m-%dT%H:%M:%S.%fZ"))}], cls=DateTimeEncoder)
        try:
        ...
        elastalert_logger.info("Alert sent to Alertmanager: {}".format(label))
